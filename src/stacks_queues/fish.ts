...
// represent N voracious fish in a river, ordered downstream along the flow of the
// river.
//
// The fish are numbered from 0 to N − 1. If P and Q are two fish and P < Q, then fish
// P is initially upstream of fish Q. Initially, each fish has a unique position.
//
// Fish number P is represented by A[P] and B[P]. Array A contains the sizes of the
// fish. All its elements are unique. Array B contains the directions of the fish. It
// contains only 0s and/or 1s, where:
//
...
// alive − the larger fish eats the smaller one. More precisely, we say that two fish
// P and Q meet each other when P < Q, B[P] = 1 and B[Q] = 0, and there are no living
// fish between them. After they meet:
//
//         If A[P] > A[Q] then P eats Q, and P will still be flowing downstream,
//         If A[Q] > A[P] then Q eats P, and Q will still be flowing upstream.
//
// We assume that all the fish are flowing at the same speed. That is, fish moving in
// the same direction never meet. The goal is to calculate the number of fish that
// will stay alive.
//
...
export function fish(A: number[], B: number[]): number {
  if (A.length !== B.length) throw new Error("Invalid A & B length");

  const downStreamFish: number[] = []; // we only store fish sizes
  let aliveCount: number = 0;

  for (let i = 0; i < A.length; i++) {
    if (B[i] === 1) {
      // Fish going downstream, push its size onto the stack
      downStreamFish.push(A[i]);
    } else {
      while (downStreamFish.length > 0) {
        const lastDownStreamFish = downStreamFish[downStreamFish.length - 1];

        if (lastDownStreamFish > A[i]) {
          // Downstream fish eats upstream fish
          break;
        } else {
          // Downstream fist gets eaten
          downStreamFish.pop();
        }
      }
  ...
  return aliveCount + downStreamFish.length;
}
